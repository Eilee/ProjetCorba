...
public class file_listImpl extends file_listPOA{
	...
	public file_listImpl(ArrayList<regular_fileImpl> files, ArrayList<directoryImpl> directories){
		list_of_directory_entry = new ArrayList<directory_entry>();
		pos = 0;
		
		Iterator<regular_fileImpl> iter = files.iterator();
		while(iter.hasNext()){
			regular_fileImpl file = iter.next();
		...
		Iterator<directoryImpl> iter2 = directories.iterator();
		while(iter2.hasNext()){
			directoryImpl directory = iter2.next();
			directory_entry de = new directory_entry();
